...
export default function ScanPage() {
  ...
  const [simVehicleKey, setSimVehicleKey] = useState<VehicleKey>("bmw_m4");
  const [scanQuery, setScanQuery] = useState("");
  const [resolvedVehicle, setResolvedVehicle] = useState<ApiVehicle | null>(null);
  ...
  const handleScanSuccess = async () => {
    ...
    if (scanQuery.trim()) {
      try {
        const response = await api.resolveVehicle(scanQuery.trim());
        setResolvedVehicle(response.vehicle);
      } catch (error) {
        setScanError(error instanceof Error ? error.message : "QR/NFT tidak ditemukan di backend.");
        return;
      }
    }
    setScanned(true);
    ...
    if (resolvedVehicle) {
      try {
        await api.createWalkinBooking({
          ...
          workshopId: currentUser?.workshopId ?? "ws-3",
          complaint: "Walk-in service from QR/NFT scan",
        });
        await syncBookings();
        router.push("/workshop/queue");
        return;
      } catch (error) {
        setScanError(error instanceof Error ? error.message : "Gagal membuat walk-in booking.");
        return;
      }
    }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      setTimeout(() => void handleScanSuccess(), 800);
    }
  };
    ...
    <div>
      ...
      {!scanned && (
        <div className="mb-8 p-4 rounded-xl border max-w-lg" style={{ background: "rgba(20,20,40,0.5)", borderColor: "rgba(94, 234, 212,0.15)" }}>
          ...
          <label className="mt-4 block text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--solana-text-muted)" }}>QR/NFT/VIN Backend Payload</label>
          <textarea
            value={scanQuery}
            onChange={(event) => setScanQuery(event.target.value)}
            rows={3}
            placeholder='Paste QR payload dari identity page, VIN, atau cNFT asset id'
            className="mt-2 w-full rounded-lg bg-black/30 px-3 py-2 text-xs text-white outline-none"
            style={{ border: "1px solid rgba(94, 234, 212,0.2)" }}
          />
          {scanError && <p className="mt-2 text-[11px] text-red-300">{scanError}</p>}
        ...
        >
          {scanMode === "nfc" ? (
            <>
              ...
              <button onClick={() => void handleScanSuccess()} className="glow-btn flex items-center gap-2 cursor-pointer">
            ...
            <>
              ...
              <div className="flex gap-3 w-full max-w-xs">
                <button onClick={() => void handleScanSuccess()} className="glow-btn flex-1 flex items-center justify-center gap-2 cursor-pointer">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
          <div className="glass-card p-8">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="p-4 rounded-xl" style={{ background: "rgba(20,20,40,0.5)" }}><p className="text-xs mb-1" style={{ color: "var(--solana-text-muted)" }}>Vehicle</p><p className="font-semibold flex items-center gap-2"><Car className="w-4 h-4" style={{ color: "var(--solana-purple)" }} /> {resolvedVehicle ? `${resolvedVehicle.make} ${resolvedVehicle.model}` : simVehicle.name}</p></div>
              <div className="p-4 rounded-xl" style={{ background: "rgba(20,20,40,0.5)" }}><p className="text-xs mb-1" style={{ color: "var(--solana-text-muted)" }}>VIN</p><p className="font-semibold mono text-sm">{resolvedVehicle?.vin ?? simVehicle.vin}</p></div>
