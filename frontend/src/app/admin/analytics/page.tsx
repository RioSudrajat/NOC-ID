...
export default function AdminAnalyticsPage() {
  ...
  const maxWsServices = wsMetrics.length > 0 ? Math.max(...wsMetrics.map(w => w.totalServices)) : 1;
          ...
          <div className="flex items-center gap-8 justify-center py-4">
            <div className="relative w-32 h-32">
              <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
                ...
                <circle cx="18" cy="18" r="15.9155" fill="none" stroke="var(--solana-green)" strokeWidth="3" strokeDasharray={`${m?.oemRate || 100} ${100 - (m?.oemRate || 100)}`} strokeLinecap="round" />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-xl font-bold">{m?.oemRate || 100}%</span>
