...
const roleColors: Record<PlatformRole, string> = {
  superadmin: "#FCA5A5",
  admin: "#5EEAD4",
  operator: "var(--solana-purple)",
  workshop: "var(--solana-green)",
  driver: "var(--solana-cyan)",
};
...
export default function AdminRolesPage() {
  ...
  const [newRole, setNewRole] = useState<PlatformRole>("driver");
  ...
  const handleAdd = () => {
    ...
    setNewWallet(""); setNewEntity(""); setNewRole("driver"); setShowAddModal(false);
  };
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        {(["superadmin", "admin", "operator", "workshop", "driver"] as PlatformRole[]).map(role => (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <div className="flex bg-black/30 p-1 rounded-xl border border-white/5 overflow-x-auto">
          {["all", "superadmin", "admin", "operator", "workshop", "driver"].map(f => (
